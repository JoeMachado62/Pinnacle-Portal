...
class IRRPHelper implements IRRPConstants {
    ...
    public function deleteLogs() {
        ...
        $nonce = ($_nonce = filter_input(INPUT_GET, "_irrp_nonce")) ? htmlspecialchars($_nonce, ENT_QUOTES, 'UTF-8') : "";
        $redirectTo = ($rTo = filter_input(INPUT_GET, "redirect_to")) ? htmlspecialchars($rTo, ENT_QUOTES, 'UTF-8') : "";
        $logType = ($log_type = filter_input(INPUT_GET, "log_type")) ? htmlspecialchars($log_type, ENT_QUOTES, 'UTF-8') : "";
            ...
            if ($redirect) {
                ...
                if ($metas["ignore_case"]) {
                    if (function_exists("mb_strtolower")) {
                        $matchQuery = mb_strtolower((string) $matchQuery);
                        $requestQuery = mb_strtolower((string) $requestQuery);
                        $request = mb_strtolower((string) $request);
                        $match = mb_strtolower((string) $match);
                    } else {
                        $matchQuery = strtolower((string) $matchQuery);
                        $requestQuery = strtolower((string) $requestQuery);
                        $request = strtolower((string) $request);
