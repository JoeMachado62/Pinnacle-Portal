        ...
        <div class="hf-column-menu">
            ...
            <div class="submenu">
                <a class="#" href="<?php echo esc_attr(add_query_arg(array('_hf_admin_action' => 'rename_data_column', '_wpnonce' => wp_create_nonce('_hf_admin_action'), 'column_key' => $key ))); ?>" onclick="var newKey = prompt('New column name', '<?php echo esc_attr($key); ?>'); if(newKey) { this.href = this.href + '&new_column_key=' + newKey; }">
                    <?php _e('Rename Column', 'html-forms-premium'); ?>
                </a>
                <a class="hf-danger" href="<?php echo esc_attr(add_query_arg(array( '_hf_admin_action' => 'delete_data_column', '_wpnonce' => wp_create_nonce('_hf_admin_action'), 'column_key' => $key ))); ?>" data-hf-confirm="<?php esc_attr_e('Are you sure you want to delete this column? All data will be lost.', 'html-forms-premium'); ?>">
                    <?php _e('Delete Column', 'html-forms-premium'); ?>
                </a>
            </div>
        </div>
        <?php
