...
class Admin
{
    public function hook()
    {
        ...
        add_action('hf_admin_form_submissions_detail', array($this, 'mark_submission_as_seen'));
    }
    ...
    public function form_settings(Form $form)
    {
        ?>
        <tr valign="top">
            <th scope="row" colspan="2" class="hf-settings-header"><?php echo __( 'Notifications', 'html-forms-premium' ); ?></th>
        </tr>

        <tr valign="top">
            <th scope="row">
                <?php _e('Enable Notifications', 'html-forms-premium'); ?>
                <a target="_blank" tabindex="-1" class="html-forms-help" href="https://htmlformsplugin.com/kb/submission-notifications/"><span class="dashicons dashicons-editor-help"></span></a>
            </th>
            <td class="nowrap">
                <label>
                ...
                <p class="description">
                    <?php _e('Select "Yes" to see a notification badge for new form submissions.', 'html-forms-premium'); ?>
                </p>
            </td>
        </tr>
        <?php
    ...
    {
        $actions['bulk_mark_submissions_as_seen'] = __('Mark as seen', 'html-forms-premium');
        $actions['bulk_mark_submissions_as_unseen'] = __('Mark as unseen', 'html-forms-premium');
        return $actions;
    }

    public function mark_submission_as_seen($submission)
    {
        if (empty($submission->form_id) || empty($submission->id)) {
            return;
        }
    ...
    public function row_actions($submission)
    {
        ...
        $form_id = absint($_GET['form_id']);
        $action_text = is_unseen($form_id, $submission->id) ? __('Mark as seen', 'html-forms-premium') : __('Mark as unseen', 'html-forms-premium');
