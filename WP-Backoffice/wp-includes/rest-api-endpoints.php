...
function paf_api_create_deal_from_app( WP_REST_Request $request ) {
    ...
    paf_add_history_entry($app_id, $deal_id, 'deal_created_from_app', "Deal created from app submission. DT Ref: {$dt_reference_number}");
...
function paf_api_update_deal_from_dealertrack( WP_REST_Request $request ) {
    ...
    $history_details = ['dt_status' => $dt_status_key, 'notes' => $notes];
    paf_add_history_entry(0, $deal_id, 'dealertrack_status_update', "DealerTrack status updated to: " . paf_get_deal_status_label($dt_status_key), $history_details);
